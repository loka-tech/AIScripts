...
# Usage: python hf_download_file.py <repo_id> <filename> <local_dir> --cache_dir <cache_dir> --branch <branch> --symlinks <true|false|auto> --fast <1|0>
# Example: python hf_download_file.py R136a1/MythoMax-L2-13B-exl2 "config.json" "/content/Mytho" --cache_dir "/content/cache" --branch "main" --symlinks "auto" --fast "1"
...
def run_hub_download(repo_id, filename, local_dir, branch="main", token=True, local_dir_use_symlinks="auto"):
    ...
    while retry and try_count < max_tries:
        try:
            hf_hub_download(filename=filename,
                            repo_id=repo_id,
                            local_dir=local_dir,
                            revision=branch,
